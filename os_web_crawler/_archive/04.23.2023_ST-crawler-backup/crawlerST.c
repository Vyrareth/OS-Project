...
int main(void){
	...
	for(int index = 0; index < 1; index++) // For loop used to loop over the seed list
	{
        ...
        URLQueue[queueTail] = (char*)malloc(strlen(trimmedURL) * sizeof(char));
        strcpy(URLQueue[queueTail], trimmedURL);
        queueTail++;
        ...
        while(queueHead < QUEUE_URL_LIMIT && queueHead < queueTail)
        {
            ...
            if (curl) { // init returns T/F value
                ...
                int numURLsFound = parseHTMLBody(currBody.bodyStrPtr, returnedURLList);

                for (int foundURLIndex = 0; foundURLIndex < numURLsFound; foundURLIndex++) {
                    // URLQueue
                    // printf("%s\n", returnedURLList[foundURLIndex]); // Used for debugging/testing
                    URLQueue[queueTail] = (char *) malloc(strlen(returnedURLList[foundURLIndex]) * sizeof(char));
                    strcpy(URLQueue[queueTail], returnedURLList[foundURLIndex]);
	...
	while(token != NULL) // This prints out the entire HTML body line-by-line
	{
		...
		if(strstr(token, "http")){
			...
			if((numURLsFromBody = numURLsFromBody + numURLsFromLine) >= PER_PAGE_URL_LIMIT){
                break;
            } // Call to function that gets URL from line
		}
		/**/
...
int getURLFromLine(char* line, char* query, char* URLSubList[])
{
    ...
    int urlBufferSize = PER_PAGE_URL_LIMIT;
    char* URLList[PER_PAGE_URL_LIMIT]; // = malloc(urlBufferSize * sizeof(char*));

	// printf("%s\n\n", "~~~~~ Below is a list of all URLs from the above line ~~~~~");
    int count = 0;
                ...
                if(strstr(queryToken, query)){ // If the "queryToken" contains "http"
                    ...
                    while(spaceToken != NULL)
                    {
                        if(strstr(spaceToken, query)){ // If the "spaceToken" contains "http"
                            if(spaceToken[0] != '.') {
                                URLSubList[count] = (char *) malloc(strlen(spaceToken) * sizeof(char));
                                strcpy(URLSubList[count], spaceToken);
                                printf("%s\n",
	...
	return count; // REMOVE, this is a placeholder
}